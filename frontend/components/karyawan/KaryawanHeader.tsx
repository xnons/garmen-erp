...
import {
    UserPlus, Search, Archive, Users, UserCheck, RefreshCw,
    LayoutList, LayoutGrid, AlignJustify, Filter
} from 'lucide-react';
import { ViewMode, DepartmentFilter } from './types';
...
export const KaryawanHeader: React.FC<KaryawanHeaderProps> = ({
    ...
    viewMode, setViewMode, deptFilter, setDeptFilter
}) => {
    const deptOptions: { id: DepartmentFilter; label: string }[] = [
        { id: 'ALL', label: 'Semua Divisi' },
        { id: 'SEWING', label: '🧵 Sewing / Jahit' },
        { id: 'CUTTING', label: '✂️ Cutting / Pola' },
        { id: 'FINISHING', label: '📦 Finishing / Pack' },
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel p-4 sm:p-6 rounded-2xl border border-slate-800">
                <div>
                    ...
                    <p className="text-slate-400 text-xs sm:text-sm mt-1">
                        Manajemen SDM, skema borongan/bulanan, log sanksi, dan tata letak data fleksibel.
                    </p>
            ...
            {/* 🎛️ Bottom Control Bar: View Switcher (Table vs Grid vs Compact) + Department Chips */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 bg-slate-950/70 border border-slate-800/80 rounded-2xl">
                {/* Department Filter Chips */}
                <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar w-full sm:w-auto">
                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider px-2 shrink-0 hidden md:inline">
                        Divisi:
                    </span>
                    {deptOptions.map((opt) => (
                        <button
                ...
                <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-800 shrink-0 self-end sm:self-auto">
                    <span className="text-[10px] font-bold text-slate-500 uppercase px-2 hidden lg:inline">Layout:</span>
                    <button
                        type="button"
                    ...
                    <button
                        ...
                        title="Tampilan Tabel Super Padat"
                        className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                            viewMode === 'COMPACT'
                                ? 'bg-indigo-600 text-white shadow-sm'
                                : 'text-slate-400 hover:text-white hover:bg-slate-800'
